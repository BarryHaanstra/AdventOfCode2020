...
let inputArray = input.lowercased().split(whereSeparator: \.isNewline)

// Map to dictionary
let bagRules = inputArray.reduce(into: [String: [String:Int]?]()) {
    let parts = $1.dropLast().components(separatedBy: "bags contain").map { $0.trimmingCharacters(in: .whitespacesAndNewlines) }
    let bagType = parts[0]

    if parts[1] == "no other bags" {
        $0[bagType] = nil
    } else {
        let bagRuleStrings = parts[1].components(separatedBy: ",").map { $0.trimmingCharacters(in: .whitespacesAndNewlines) }
        
        let bagRules = bagRuleStrings.reduce(into: [String: Int]()) {
            let parts = $1.components(separatedBy: " ")
            let amount = Int(parts[0])!
            let bagType = parts.dropFirst().dropLast().joined(separator: " ")
            $0[bagType] = amount
        }
...
// MARK: - Question 1
func findMatches(forColor color: String) -> [String] {
    let filteredRules = bagRules.filter {
        guard let rules = $0.value else {
            return false
        }
        return rules.keys.contains(color)
    }
    return filteredRules.map { $0.key }
}

func findMatchesRecursive(forColor color: String) -> [String] {
    var matches = findMatches(forColor: color)
    for color in matches {
        matches.append(contentsOf: findMatchesRecursive(forColor: color))
    }
    return Array(Set(matches)) // Filter out doubles
}

print(findMatchesRecursive(forColor: "shiny gold").count)


// MARK: - Question 2
func countBagsInside(color: String) -> Int {
    var count = 0
    guard let bag = bagRules[color], let rules = bag else {
        return count
    }

    for (color, amount) in rules {
        count += amount
        count += amount * countBagsInside(color: color)
    }

    return count
}

print(countBagsInside(color: "shiny gold"))
