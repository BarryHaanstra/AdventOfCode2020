...
// Split string by double newlines to get groups
let groupData = input.lowercased().components(separatedBy: "\n\n")

// MARK: - Question 1
let groupResults1 = groupData.map {
    String(Set($0.filter {
        "abcdefghijklmnopqrstuvwxyz".contains($0)
    }))
}
print(groupResults1.map { $0.count }.reduce(0) { $1 + $0 })


// MARK: - Question 2
let groupResultsSets = groupData.map {
    $0.components(separatedBy: "\n").map { Set($0) }
}
let intersectedResults = groupResultsSets.map {
    $0.reduce($0.first!) { $0.intersection($1) }
}
print(intersectedResults.map { $0.count }.reduce(0) { $1 + $0 })
