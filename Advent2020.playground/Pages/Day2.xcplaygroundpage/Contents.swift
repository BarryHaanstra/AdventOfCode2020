...
struct PasswordLine {
    ...
    func isValidQuestion1() -> Bool {
        let characterCount = password.reduce(0) {
            $1 == character ? $0 + 1 : $0
        }
        return (characterCount >= min && characterCount <= max)
    }
    
    func isValidQuestion2() -> Bool {
        ...
        let char1 = password[firstIndex]
        let char2 = password[secondIndex]
        return ((char1 == character && char2 != character) || (char2 == character && char1 != character))
    }
}

// Split string by newlines, get password definitions
let inputArray = input.split(whereSeparator: \.isNewline)

// Convert to passwordline structs so we can start using the data
let passwordLines: [PasswordLine] = inputArray.map { line in
    let inputParts = line.split(separator: " ")
    let amounts = inputParts[0].split(separator: "-").map { Int($0)! }
    let minValue = amounts[0]
    let maxValue = amounts[1]
    let characterValue = inputParts[1].first!
    let passwordValue = String(inputParts[2])
    return PasswordLine.init(min: minValue, max: maxValue, character: characterValue, password: passwordValue)
}

// MARK: - Question 1
print(passwordLines.filter { $0.isValidQuestion1() }.count)

// MARK: - Question 2
print(passwordLines.filter { $0.isValidQuestion2() }.count)
